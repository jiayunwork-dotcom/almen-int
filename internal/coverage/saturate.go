...
var satScratch []float64

func PutSatScratch(v float64) {
	if satScratch == nil {
		satScratch = make([]float64, 1)
	}
	satScratch = satScratch[:1]
	satScratch[0] = v
}

func SatScratch() float64 {
	if len(satScratch) == 0 {
		return 0
	}
	return satScratch[0]
...
func HasSatScratch() bool {
	return len(satScratch) > 0
}
