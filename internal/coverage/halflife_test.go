...
func TestCoverageTable(t *testing.T) {
	...
	// After six half-lives the coverage must be above 98%.
...
func TestRelativeHeadroom(t *testing.T) {
	kappa := 2.6
	// At coverage 0.5 the relative headroom is above 10% (not saturated).
	if !(RelativeHeadroom(kappa, 0.5) > 0.10) {
		t.Errorf("relative headroom at 0.5 must exceed 10%%, got %g", RelativeHeadroom(kappa, 0.5))
	}
	// At coverage 0.98 the relative headroom is below 10% (saturated).
	...
	// The headroom sign must agree with the saturation predicate.
...
func TestCoverageFractionAt(t *testing.T) {
	...
	// As time grows the fraction must approach one from below.
	prev := 0.0
