...
func TestTriangleMomentBelowUniform(t *testing.T) {
	p := testParams()
	if !TriangleLeverPositive(p) {
		t.Fatal("A-strip layer must sit above the triangle centroid")
	}
	u := BendingMoment(p)
	tri := TriangleBendingMoment(p)
	if tri >= u {
		t.Fatalf("same peak stress: triangle moment must be below uniform, tri=%g uniform=%g", tri, u)
	}
	if ProfileForceRatio(p) != 0.5 {
		t.Fatalf("triangle force must be half the uniform layer, got %g", ProfileForceRatio(p))
	}
	hTri := TrianglePlateauArcHeight(p)
	hUni := PlateauArcHeight(p)
	if hTri >= hUni {
		t.Fatalf("triangle plateau must sit below uniform: %g vs %g", hTri, hUni)
	}
	peak, err := UniformEquivalentPeak(p)
	if err != nil {
		t.Fatal(err)
	}
	if peak <= ResidualStress(p) {
		t.Fatalf("matching uniform moment needs a higher triangle peak, got %g vs %g", peak, ResidualStress(p))
	}
}

func TestTwoSidedBalanceCancelsArc(t *testing.T) {
	p := testParams()
	h, err := TwoSidedPlateau(p, 0.9, 0.9)
	if err != nil {
		t.Fatal(err)
	}
	if !closeEnough(h, 0, 1e-12) {
		t.Fatalf("equal gains must cancel curvature, got %g", h)
	}
	if !GainsBalanced(0.9, 0.9, 1e-12) {
		t.Fatal("equal gains must be balanced")
	}
	one, err := OneSidedMatchesPlateau(p, 1)
	if err != nil {
		t.Fatal(err)
	}
	if !closeEnough(one, PlateauArcHeight(p), 1e-12) {
		t.Fatalf("one-sided unit gain must match plateau: %g vs %g", one, PlateauArcHeight(p))
	}
	ok, err := FlipSidesNegatesMoment(p, 0.8, 0.3)
	if err != nil {
		t.Fatal(err)
	}
	if !ok {
		t.Fatal("swapping faces must negate net moment")
	}
	front, err := TwoSidedPlateau(p, 0.8, 0.2)
	if err != nil {
		t.Fatal(err)
	}
	back, err := TwoSidedPlateau(p, 0.2, 0.8)
	if err != nil {
		t.Fatal(err)
	}
	if !closeEnough(front, back, 1e-12) {
		t.Fatalf("arc height is unsigned: front=%g back=%g", front, back)
	}
}
