...
// RenderOptions controls how the text renderer formats a result.
type RenderOptions struct {
	Verbose bool // print the intermediate quantities of the model and coverage chain
}

// RenderResult formats a full result as plain text. The output always carries
// the arc height, the coverage, the saturation state and the recommended
// strip; the verbose flag appends the intermediate quantities and the
// dimensionless regime checks.
...
// recommendedText renders the recommended-strip line, including the
// saturation gating: no letter is printed for an unsaturated process.
...
// width is the label column width used by Labeled throughout the report.
const width = 18

// F3 formats a value with three decimals.
