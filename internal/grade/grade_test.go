...
// testParams mirrors the reference case used elsewhere in the project.
...
func TestRecommendBandSelection(t *testing.T) {
	cases := []struct {
		...
		{0.05, "N"},
		{0.10, "A"}, // lower edge of the A band, inclusive
		{0.44, "A"},
		{0.60, "C"}, // upper edge of the A band, exclusive
		{0.90, "C"},
	}
...
func TestGradeGatingWhenNotSaturated(t *testing.T) {
	...
	// The gating must also hold on the assembled result.
...
func TestBandsPartitionTheAxis(t *testing.T) {
	...
	// The A strip is the standard medium band used by the reference case.
	a, ok := ByLetter("A")
	if !ok {
